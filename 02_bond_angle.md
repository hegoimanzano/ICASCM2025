...
To solve the charge problem, count the numer of atoms of each element (M), multiply it by its formal charge (qj), and sum over all the elements (N). The sum (Q) must be zero, else, divide the resulting charge by all the oxygen atoms:
