...
**3. Simulation Control and Execution.** Now we start the simulation. In any MD simulation there are at least two steps. First, an **equilibration phase**, in which our system adapts to the desired thermodynamic conditions. First, we do an energy minimization (`minimize`) to relax the atomic positions and avoid any "unphysical" configuration that the structure generation may have provided. Second, we assign random `velocity` to the particles according to a Boltzmann distribution at 298 K. Then, we perform a MD equilibration in the isothermal-isobaric ensemble `fix npt`, at an initial and final temperature and pressure of 298 K and 1 atm, applying a dumping parameter of the thermostat and a barostat every 100 and 1000 steps, respectively, to maintain the target temperature and pressure. We record selected `thermo` properties in the output every 1000 steps. Finally, we need to `unfix` the fix that was defined for this phase to prepare the system for the next stage.
